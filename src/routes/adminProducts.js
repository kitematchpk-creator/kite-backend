...
function buildPayload(req) {
  ...
  const galleryFiles = unifiedImageFiles.length ? unifiedImageFiles.slice(1) : (files.images || []);
  ...
  return {
    ...
    image: primaryImageFile ? buildFileUrl(req, primaryImageFile) : body.image || '',
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      if (!req.files?.images?.length) data.images = Array.isArray(data.images) && data.images.length
        ? data.images
        : (existing.images || []);
